...
const Carrusel: React.FC<CarruselProps> = ({ imagenes, itemsPorDesplazamiento = 4 }) => {
  ...
  const cambiarImagen = (n: number) => {
    ...
    const nuevoIndice = (indice + n * itemsPorDesplazamiento + imagenes.length) % imagenes.length;
    ...
    <div className="relative w-full overflow-hidden">
      ...
      >
        {imagenesCiclo.map((imagen, idx) => (
          <div key={idx} className="flex-shrink-0 w-1/4"> {/* Cada imagen ocupa el 25% del ancho */}
            <img src={imagen} alt={`Imagen ${idx + 1}`} className="w-full h-auto object-cover" />
          </div>
        ))}
      </div>
      ...
      <button
        className="absolute left-0 top-1/2 transform -translate-y-1/2 p-3 bg-black bg-opacity-50 text-white rounded-full"
      ...
      <button
        className="absolute right-0 top-1/2 transform -translate-y-1/2 p-3 bg-black bg-opacity-50 text-white rounded-full"
